...
A good encryption scheme must also include resistance to Man-in-the-Middle attacks. Basically, a client must
...
with particular arguments any number of times will cause at most one change to the server filesystem.
...
same position from the same file will always return the same result - and in case the filesystem is modified
externally between the calls, the client can simply rely on the newest result. The same holds for listing
directories, querying file attributes etc.  Setting metadata values or writing same data into the same file
offset repeatedly is also idempotent.  However, an {\it append} operation is not, because it writes to current
end of file, which might have been changed by the previous {\it append}.
...
Similar is true for creating files\footnote{Unless creating is implicit in a write operation, in which case
the write operation discussion applies.} and directories. Once the first create operation succeeds, subsequent
applications will fail because the object already exists. There is an important caveat, though: in the absence
of better locking mechanisms, some programs use temporary files and especially directories to indicate that
a lock on a particular resource is held. For this usecase, it is very important to distinguish whether the
create operation succeeded (the caller now holds the lock) or failed (somebody else holds the lock). This is
virtually impossible in a stateless protocol, and difficult in a stateful one as well. The server would need
to retain state even for some time after the connection is lost, it would have to match reconnection attempts
to previously lost connections, recognize that a seemingly live connection on the server is actually already
lost on the client's end and so on. Protocols might choose not to support this scenario - even if done
properly, directory-based locking is prone to deadlocks anyway, so it might be more efficient to spend efforts
on a proper locking mechanism.
...
Specialized protocols, or protocols designed specifically for filesystem-like access, may prescribe more
specific caching policies or even facilitate mechanisms for cache invalidation triggered by the server. As an
example of one of the simpler approaches, the client can request to be notified of all file changes. A more
complicated example is the server giving virtual ownership of a file to a client, and notifying it when other
clients want to access the file so that the virtual owner can sync back its changes. Note that it is
significantly easier for the server software to maintain client caches and concurrent access invariants if it
is the only process that works with the published filesystem.
