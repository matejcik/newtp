...
\begin{description}[leftmargin=1.7cm,style=sameline]
	...
	\ttitem{data} Byte sequence of arbitrary length. Information about its length will be provided
		separately -- either in a different field, or obtained by a~calculation.
...
\begin{tabular}{|lllcp{7.7cm}|}
	\hline
	Code & Name & Type & W & Value \\
	\hline
	...
	\tline{0x04}{dev\_id}{uint32} & No &
		Arbitrary ID of the filesystem tree in which this entry resides.
		Entries within the same filesystem will have the same {\tt dev\_id}
	...
	\tline{0x06}{atime}{time\_t} & Yes & Last access time \\
	\tline{0x07}{mtime}{time\_t} & Yes & Last modification time \\

	\hline
	\multicolumn{5}{|c|}{\textit{POSIX-specific attributes}} \\
	\hline

	\tline{0x10}{ptype}{byte} & No & File type TODO \\
	\tline{0x11}{perms}{uint16} & Yes & File permission bits \\
	...
	\tline{0x20}{attrib}{uint32} & Yes & Windows file attributes \\
