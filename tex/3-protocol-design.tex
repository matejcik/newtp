...
Two standard extensions, {\tt links} and {\tt replay}, are specified in this work.
...
\begin{description}[noitemsep,topsep=1pt]
	\item[Result data:] {\tt data attribute\_values}
...
This extension provides commands for manipulating symbolic links and hard links. It specifies the following
new attribute:

\begin{tabular}{llp{7cm}}
	\pkfield{0x40}{byte type\_with\_link}{Same as {\tt type}, but its value can be {\tt 0x03} to indicate
	symbolic link}
\end{tabular}

The extension also specifies two error codes:

\begin{tabular}{lll}
	\pkfield{0x80}{LINKS\_E\_OUTSIDE}{Symbolic link points outside of published root}
	\pkfield{0x81}{LINKS\_E\_NOTLINK}{Not a symbolic link}
\end{tabular}

\subsection{MAKE\_LINK} %%%%%%%%%%%%
...
Creates a new file at {\tt newpath} a hardlink to the file handle. File handle must refer to a file. {\tt
newpath} must not exist, and the directory containing it must be on the same filesystem as the file handle.
...
\begin{description}[noitemsep,topsep=1pt]
	...
	\item[Specific errors:] \hfill \\
		\begin{tabular}{lp{10.5cm}}
			{\tt E\_BADPATH} & File handle path or the new path is not acceptable \\
			{\tt E\_EXISTS} & {\tt newpath} points to an existing object \\
			{\tt E\_CROSSDEV} & {\tt newpath} is on a different filesystem \\
		\end{tabular}
\end{description}
...
Same as {\tt STAT}, except that if the file handle points to a symbolic link, queries the symbolic link entry
...
\begin{description}[noitemsep,topsep=1pt]
	...
	\item[Specific errors:] \hfill \\
		\begin{tabular}{lp{10.5cm}}
			{\tt E\_BADATTR} & Attribute code from the list is unknown or not supported
				on the platform, or an attribute is listed more than once \\
		\end{tabular}
\end{description}

\subsection{MAKE\_SYMLINK} %%%%%%%%%%%%%%

\begin{description}[noitemsep,topsep=1pt]
	\item[Code:] {\tt 0x02}
	\item[Arguments:] {\tt data symlink}
\end{description}

Creates a new symbolic link at the file handle, pointing to {\tt symlink}. {\tt symlink} may be an absolute
path, which is interpreted by the server as relative to exported root. It may also be a relative path, in
which case it is interpreted literally, but it must not escape the exported root at any point during its
resolution (i.e., ``{\tt ../backToRoot/something}'' is not allowed). The file handle must not point to an
existing object.
...
\begin{description}[noitemsep,topsep=1pt]
	...
	\item[Specific errors:] \hfill \\
		\begin{tabular}{lp{9cm}}
			{\tt E\_BADPATH} & File handle path or content of {\tt symlink} is not an acceptable
				path \\
			{\tt LINKS\_E\_OUTSIDE} & {\tt symlink} is a relative path and points outside of
				exported root \\
			{\tt E\_EXISTS} & File handle points to an existing object \\
		\end{tabular}
\end{description}
...
Returns value of symbolic link, i.e., the path that it points to. If the link points outside the exported
root, {\tt LINKS\_E\_OUTSIDE} is returned. If the link is absolute, the value is returned as an absolute path
from the filesystem root. If the link is relative, the value is returned literally.

\begin{description}[noitemsep,topsep=1pt]
	\item[Result data:] {data link\_value}
	\item[Specific errors:] \hfill \\
		\begin{tabular}{lp{10.5cm}}
			{\tt LINKS\_E\_OUTSIDE} & Link points outside the exported root \\
			{\tt LINKS\_E\_NOTLINK} & File handle does not refer to a symbolic link \\
		\end{tabular}
\end{description}

%%%%%%

\section{Replay extension}

{\bf Name:} {\tt replay} \\
{\bf Extension code:} {\tt 0x02 EXT\_REPLAY}

The principle behind this extension was explained in section~\ref{ssec:replay}. To restate, a client registers
with the server and receives a unique token. If a network error occurs, the client can reconnect and use the
token to request a replay of last operations accepted by the server.

The replay cache stores reply headers (without payloads) from non-idempotent requests. For the core set, this
means the following commands:
\begin{itemize}[nolistsep]
	...
	\item {\tt MAKEDIR}
\end{itemize}
Extensions can add their own non-idempotent commands to the list. {\tt WRITE} is explicitly excluded, as it is
a frequently repeated operation, and simply copying a huge file to the server could effectively overwrite the
whole replay cache.

Size of the replay cache is implementation-defined, but it must hold at least 10~items. It is recommended that the
server saves the replay cache on a permanent storage, so that it can be used even in case of server crash.

Malicious actors could abuse this feature by creating and killing a huge number of connections in rapid
succession, and thus filling server memory or storage space by replay caches. To prevent this, the server
should place a limit on total size of all replay caches. When this limit is reached, the server may delete
replay caches for disconnected sessions, oldest first. The server should also delete old replay caches
periodically. A replay cache is considered old when its associated session has been dead for over
10 minutes.

Before replaying a session, the server must check that the supplied token belongs to the same user account.
This obviously does not apply to anonymous mode -- all anonymous clients are considered to belong to the same
account.

The extension does not define any attributes. It defines the following special error code:

\begin{tabular}{lll}
	\pkfield{0x80}{REPLAY\_E\_BADTOKEN}{Invalid session token}
\end{tabular}
...
Assigns a random identifier to the session, and starts recording non-idempotent commands. If called
repeatedly, returns the same identifier.

\begin{description}[noitemsep,topsep=1pt]
	\item[Result data:] \hfill \\
		\begin{tabular}{lll}
			\pkfield{uint16}{cache\_size}{Size of the replay cache}
			\pkfield{uint64}{token}{Unique token for the session}
		\end{tabular}
	\item[Specific errors:] none
\end{description}
...
\begin{description}[noitemsep,topsep=1pt]
	\item[Code:] {\tt 0x01}
	\item[Arguments:] {\tt uint64 token}
\end{description}

Connects this session to the supplied token. If a different live session is associated with the token, kills
that session.  Returns contents of the replay cache for this token, and resumes recording.

If the session token is unknown, or the referenced session does not belong to this user account, does nothing
and returns {\tt REPLAY\_E\_BADTOKEN}.

\begin{description}[noitemsep,topsep=1pt]
	\item[Result data:] \hfill \\
		\begin{tabular}{lll}
			\pkfield{uint16}{num\_items}{Number of operations in the replay cache}
			\pkfield{data}{items}{Sequence of reply packet headers}
		\end{tabular}
	\item[Specific errors:] \hfill \\
		\begin{tabular}{lp{10.5cm}}
			{\tt REPLAY\_E\_BADTOKEN} & Invalid session token \\
		\end{tabular}
\end{description}
